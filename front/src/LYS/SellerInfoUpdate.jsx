...
function SellerInfoUpdate() {
    ...
    const [sellerPass2, setSellerPass2] = useState('');
    const [sellerTel, setSellerTel] = useState('');
    const [sellerAddrNum, setSellerAddrNum] = useState('');
    const [sellerAddrJibun, setSellerAddrJibun] = useState('');
    const [sellerAddrRoad, setSellerAddrRoad] = useState('');
    const [sellerAddrDetail, setSellerAddrDetail] = useState('');
    const addrChangeList = {
        setAddrNum: setSellerAddrNum,
        setAddrRoad: setSellerAddrRoad,
        setAddrJibun: setSellerAddrJibun
    };
    const onChangeSellerPass = useCallback(e => {
        setSellerPass(e.target.value);
    }, []);

    const onChangeSellerPass2 = useCallback(e => {
        setSellerPass2(e.target.value);
    }, []);

    const onChangeSellerTel = useCallback(e => {
        setSellerTel(e.target.value);
    }, []);

    const onChangeSellerAddrDetail = useCallback(e => {
        setSellerAddrDetail(e.target.value);
    }, []);
    ...
    useEffect(() => {
        return async () => {
            ...
            console.log(data);
            setSellerInfo(data);
            setSellerPass(data.sellerPass);
            setSellerPass2(data.sellerPass2);
            setSellerTel(data.sellerTel);
            setSellerAddrNum(data.sellerAddrNum);
            setSellerAddrJibun(data.sellerAddrJibun);
...
// 비밀번호 확인이 비어있을때
    ...
    // 전화번호 '-'방지
    $(function () {
        $("#sellerTel").on("blur keyup", function () {
    ...
    return (
        <WrapLogin>
            ...
            <ReauthPhone>
                <LoginWrap>
                    <LoginSection>
                        <LoginTitle>회원정보수정 (판매자 회원용)</LoginTitle>
                        ...
                        <form action={'/sellerInfoUpdate'} method={'post'}>
                            <FormBlock>
                                ...
                                <FormBlockBody>
                                    <InputTextSizeW>
                                        <Input type="email" name={'sellerId'} id={'sellerId'}
                                               value={sellerInfo?.sellerId ? sellerInfo.sellerId : null}
                                               readOnly={true}
                            ...
                            <FormBlock>
                                ...
                                <FormBlockBody>
                                    <InputTextSizeW>
                                        <Input type={'string'} name={"sellerPass"} id={"sellerPass"} onClick={() => {
                                            checkPw();
                                            checkDoublePw();
                                            checkList[1] = false;
                                        }}
                                ...
                                <FormBlockBody>
                                    <InputTextSizeW>
                                        <Input type={'string'} placeholder="변경할 비밀번호 확인" id={"sellerPass2"}
                                               onChange={(e) => {
                                                   onChangeSellerPass2(e);
                                                   checkPw2();
                                                   checkDoublePw();
                                               }}/>
                            ...
                            <FormBlock>
                                ...
                                <FormBlockBody>
                                    <InputTextSizeWTypeL>
                                        <Input type="text" name={"sellerName"} id={"sellerName"} readOnly={true}
                                               value={sellerInfo?.sellerName ? sellerInfo.sellerName : null}
                                               style={{backgroundColor: "#c8c8c8"}}
                                        />
                                    </InputTextSizeWTypeL>
                                </FormBlockBody>
                            ...
                            <FormBlock>
                                ...
                                <FormBlockBody>
                                    <InputTextSizeW>
                                        {/* 생일 */}
                                        <Input name={"sellerBirth"} style={{width: 241, backgroundColor: "#c8c8c8"}}
                                               maxLength={6} id={"sellerBirth"} readOnly={true}
                                               value={sellerInfo?.sellerBirth ? sellerInfo.sellerBirth : null}
                                               className={"col-6"}/>
                                        &nbsp;-&nbsp;
                                        {/* 성별 */}
                                        <Input name={"sellerGender"} style={{width: 50, backgroundColor: "#c8c8c8"}}
                                               maxLength={1} id={"sellerGender"} readOnly={true}
                                               value={sellerInfo?.sellerGender ? sellerInfo.sellerGender : null}
                                               className={"col-6"}/>
                                        &nbsp;*&nbsp;*&nbsp;*&nbsp;*&nbsp;*&nbsp;*
                                    </InputTextSizeW>
                                </FormBlockBody>
