...
const Login = () => {
    ...
    const userLoginCheck = async () => {
        ...
        const {data} = await axios.post(`/${loginChange}`, null, {
            params: {
                id: id, pass: pass
            }
        })
        ...
        <>
            <LoginWrap>
                <LoginContainer>
                    ...
                    <BorderAndText>
                        <Link
                            onClick={() => loginChange == "userLogin" ? setLoginChange("sellerLogin") : setLoginChange("userLogin")}>{location?.state?.pathname == null ?
                            <span>{loginChange == "userLogin" ? "사업자 로그인" : "일반회원 로그인"}</span> : null}</Link>
                    </BorderAndText>
